...
describe('Thai admin live source contract', () => {
  ...
  it('exposes a public spectator board route and API without admin auth', () => {
    ...
    expect(board).toContain('Места внутри пула по раундам');
    expect(board).toContain('Здесь показано не общее место по всему турниру');
    expect(board).toContain('function groupProgressRowsByPool');
    expect(board).toContain('progressGroups.map((group) => (');
    expect(board).toContain('Раунд 1');
    expect(board).toContain('Раунд 2');
    expect(board).toContain('Раунд 1 · идёт');
    expect(board).toContain('Раунд 2 · завершён');
    expect(board).toContain('function formatRoundPlace');
    expect(board).toContain('function renderPlaceShift');
    expect(board).toContain('Раунд 1 → Раунд 2');
    expect(board).toContain('→');
    expect(board).toContain('↑ +');
    expect(board).toContain('без изменений');
