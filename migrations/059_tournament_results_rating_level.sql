ALTER TABLE tournament_results
  ADD COLUMN IF NOT EXISTS rating_level TEXT;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tournament_results_rating_level_check'
  ) THEN
    ALTER TABLE tournament_results
      ADD CONSTRAINT tournament_results_rating_level_check
      CHECK (rating_level IS NULL OR rating_level IN ('hard', 'advance', 'medium', 'lite'));
  END IF;
END $$;

UPDATE tournament_results tr
SET rating_level = CASE
  WHEN LOWER(COALESCE(t.level, '')) IN ('advance', 'advanced') THEN 'advance'
  WHEN LOWER(COALESCE(t.level, '')) = 'medium' THEN 'medium'
  WHEN LOWER(COALESCE(t.level, '')) IN ('lite', 'light', 'easy') THEN 'lite'
  ELSE 'hard'
END
FROM tournaments t
WHERE t.id = tr.tournament_id
  AND tr.rating_level IS NULL;
  ...
  LOOP
    ...
    VALUES
      (v_trn_id, v_player.id,
       ...
       END,
       CASE
         WHEN LOWER(COALESCE(v_rec.rating_level, '')) IN ('advance', 'advanced') THEN 'advance'
         WHEN LOWER(COALESCE(v_rec.rating_level, '')) = 'medium' THEN 'medium'
         WHEN LOWER(COALESCE(v_rec.rating_level, '')) IN ('lite', 'light', 'easy') THEN 'lite'
         ELSE 'hard'
       END)
