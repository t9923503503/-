...
import { listMyPlayInvites } from '@/lib/play-service';
import { playErrorResponse } from '@/lib/play-http';

export const dynamic = 'force-dynamic';

// Входящие приглашения игрока (TZ §4)
export async function GET(req: NextRequest) {
  const user = getPlayUserFromRequest(req);
  if (!user) return NextResponse.json({ error: 'Требуется вход в аккаунт' }, { status: 401 });
  try {
    return NextResponse.json(await listMyPlayInvites(user.id));
  } catch (error) {
    return playErrorResponse(error, 'invites.get');
  }
}
