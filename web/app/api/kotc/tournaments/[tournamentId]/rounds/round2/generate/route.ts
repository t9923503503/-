...
import { generateKotcRound2, isKotcTournamentError } from '@/lib/kotc-live/tournament';
...
export async function POST(
  ...
  try {
    ...
    const result = await generateKotcRound2(tournamentId);
    return NextResponse.json({ ...result, serverNow: Date.now() });
  } catch (error) {
    return errorResponse(error, 'round2.generate.post');
  }
}
