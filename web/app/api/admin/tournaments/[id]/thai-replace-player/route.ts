...
import { isThaiJudgeError, replaceThaiTournamentPlayer } from '@/lib/thai-live';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!oldPlayerId) {
      return NextResponse.json({ error: 'oldPlayerId is required' }, { status: 400 });
    }
    if (!newPlayerId) {
    ...
    const replacement = await replaceThaiTournamentPlayer(id, { oldPlayerId, newPlayerId });
    await writeAuditLog({
      actorId: auth.actor.id,
      actorRole: auth.actor.role,
      action: 'tournament.thaiReplacePlayer',
      ...
      afterState: {
        ...
        roundsTouched: replacement.roundsTouched,
        matchesTouched: replacement.matchesTouched,
    ...
    return adminErrorResponse(error, 'tournaments.thaiReplacePlayer');
  }
}
